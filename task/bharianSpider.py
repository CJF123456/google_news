...
class AntaranewsSpider(object):
    def __init__(self):
        # 来源
        self.mmd5 = 'google:bharian'
        self.project_name = self.__class__.__name__
        self.web_name = "马来西亚每日新闻"
        self.first_url = "https://www.bharian.com.my"
    ...
    def parse_info(self, column_first, column_second, kw_site, list_url, pc_headers, source_id):
        ...
        if st:
            html = etree.HTML(con)
            els = html.xpath(
                '//div[@class="row"]/div/a|//div[@class="row"]/div/h3/a')
            for el in els:
                try:
    ...
    def get_content_html(self, html):
        ...
        con_htmls = []
        for divcon in soup.select('div.field.field-body'):
